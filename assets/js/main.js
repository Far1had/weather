...
function displayWeather(data) {
    ...
    if (data && data.name && data.main && data.weather && data.sys && data.coord) {
        ...
        if (data.main.temp !== undefined) {
            const temperatureValue = Math.round(data.main.temp - 273.15);
            temperatureElement.innerHTML = `<div id="temperatureValue" style="font-size: ${calculateFontSize(temperatureValue)};">${temperatureValue}°C</div>`;
        } else {
        ...
        const localTime = new Date(data.dt * 1000 + data.timezone * 1000); // Berücksichtige die Zeitzone
